...
##    """
##    __constraintsThatCannotBeHandled__ = ['c', 'h', 'Aeq', 'A']
...
##        P.chromoMinValues = p.lb.tolist() # mb tolist?
##        
##        #maximum values the genes can take
##        P.chromoMaxValues = p.ub.tolist() # mb tolist?
##        
##        #use integers instead of floats
##        P.useInteger = 0 # TODO: modify me!
##        
##        #crossover
...
##        #for 50 epochs
##        
##        if p.iprint >= 0: print 'itn', 0, ': Fk=%0.4e' % atleast_1d(asfarray(p.f(p.x0)))[0]
##        
...
##          if (p.iprint>0 and itn>0 and itn % p.iprint == 0) or (p.istop and p.iprint>=0):
##              print 'itn', itn, ' Fk=%0.4e' % atleast_1d(asfarray(fval))[0], ' Fbest=%0.4e' % atleast_1d(asfarray(p.ff))[0]
##          if p.istop: 
##              #p.xk,  p.fk = p.xf,  p.ff
##              return
