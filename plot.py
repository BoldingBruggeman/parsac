...
if options.database is None:
   ...
   if not os.path.isfile(options.database):
       print 'SQLite database %s does not exist.' % options.database
       sys.exit(1)
...
def update(fig=None):
    ...
    if fig is not None:
        ...
        if options.update: fig.canvas.mpl_connect('close_event', lambda evt: sys.exit(0))
    ...
    if not options.update: pylab.savefig('estimates.png',dpi=300)
...
def countAllResults():
    c = db.cursor()
    count = c.execute("SELECT COUNT(*) FROM results").fetchone()[0]
    c.close()
    return count
...
if options.update:
    ...
    while 1:
        ...
        while 1:
            pylab.pause(5.)
            newcount = countAllResults()
            if newcount!=count:
                print '%i new.' % (newcount-count)
                count = newcount
                break
else:
    update()
