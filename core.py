...
class TransformedProblem:
    ...
    def transform(self,parameters):
        if not numpy.any(self.logtransform): return parameters
        assert len(parameters)==len(self.logtransform)
        vals = []
        for v,tf in zip(parameters,self.logtransform):
            if tf and v is not None: v = numpy.log10(v)
            vals.append(v)
    ...
    def untransform(self,parameters):
        ...
        vals = []
        for v,tf in zip(parameters,self.logtransform):
            if tf and v is not None: v = 10.**v
            vals.append(v)
        return numpy.array(vals)
...
class ReportingProblem:
    ...
    def evaluateFitness(self,parameters):
        ...
        if self.reportfunction is not None: self.reportfunction(parameters,fitness)
        return fitness
...
class Optimizer:
    ...
    def run(self,method=SIMPLEX,par_ini=None,par_min=None,par_max=None,logtransform=None,modules=(),maxiter=1000,maxfun=1000,verbose=True,reltol=0.01,abstol=1e-8,popsize=None,maxgen=500,F=0.5,CR=0.9,initialpopulation=None,parallelize=True,ppservers=(),ncpus=None):
        ...
        if method!=DIFFERENTIALEVOLUTION: problem = ReportingProblem(problem,self.reportfunction)

        problem = TransformedProblem(problem,logtransform=logtransform)
        if par_min is not None: par_min = problem.transform(par_min)
        if par_max is not None: par_max = problem.transform(par_max)
        if par_ini is not None: par_ini = problem.transform(par_ini)
    ...
    def profile(self,start,maxstep=None,targetdelta=None,profile=None,nzoom=4,logtransform=None):
        if profile is None: profile = numpy.ones((len(start),),dtype=bool)
        if targetdelta is None: targetdelta = 0.5*scipy.stats.chi2.ppf(0.95,1)
        ...
        def run(ipar,x,step,target):
            while 1:
                ...
                x_ini = list(x)
                del x_ini[ipar]
                if not numpy.isfinite(ev(x_ini)): return x
                est = scipy.optimize.fmin(ev,x_ini,disp=False)
                if -ev(est)<target: return x
