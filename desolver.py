...
if pp is not None:
    ...
    for v,tv in zip(ver,map(int,minppversion.split('.'))):
        ...
        if v<tv:
            ...
            print 'Parallel Python supprot for Differential Evolution is disabled.'
            pp = None
    ...
    def Solve(self):
        ...
        if pp is not None:
            ...
            if self.populationSize!=jobsperworker*nworkers:
                if self.verbose: print 'Setting population size to %i (was %i) to ensure it is a multiple of number of workers (%i).' % (jobsperworker*nworkers,self.populationSize,nworkers)
        ...
        try:
            ...
            for igeneration in range(self.maxGenerations):
                ...
                for itarget,trial in enumerate(trials):
                    ...
                    if self.reporter is not None: self.reporter.reportResult(trial,trialfitness)
                    ...
                    if trialfitness>=fitness[itarget]:
                        ...
                        if ibest is None or trialfitness>fitness[ibest]: ibest = itarget
                ...
                if (currange<=tol).all() or (self.minfitnesstoreach is not None and fitness[ibest]>self.minfitnesstoreach): break
